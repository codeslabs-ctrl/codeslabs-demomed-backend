...
const CHAT_TOOLS = [
  ...
  { type: "function" as const, function: { name: "crear_recipe_medico", description: "Genera **dos PDF** separados: (1) récipe solo con **nombres** de medicamentos, una línea por medicamento; (2) indicaciones con el texto completo para el paciente. Solo rol médico; requiere consulta completada/finalizada contigo. Pide al médico nombres (p. ej. Acetaminofén) y el párrafo de indicaciones; opcionalmente el detalle con dosis en texto libre no va al PDF de nombres. Tras éxito: botones PDF medicamentos y PDF indicaciones.", parameters: { type: "object", properties: { paciente_id: { type: "number" }, paciente_nombre: { type: "string" }, nombres_medicamentos: { type: "string", description: "Un medicamento por línea, solo nombre (p. ej. Acetaminofén\\nLoratadina). Es lo único que verá el PDF de medicamentos." }, texto_indicaciones: { type: "string", description: "Indicaciones completas para el paciente (segundo PDF)." }, texto_recipe: { type: "string", description: "Opcional: dosis y pauta en texto; no se incluye en el PDF de medicamentos." }, fecha_emision: { type: "string", description: "YYYY-MM-DD opcional" } }, required: ["nombres_medicamentos", "texto_indicaciones"] } } },
...
- Récipe PDF desde el chat: \`crear_recipe_medico\` con paciente + **nombres_medicamentos** (una línea por nombre) + **texto_indicaciones**. Se generan dos PDF. Si falla por consulta no completada, explica y ofrece \`open_section\`. Tras éxito, menciona **PDF medicamentos** y **PDF indicaciones**.

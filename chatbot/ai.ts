...
const CHAT_TOOLS = [
  ...
  { type: "function" as const, function: { name: "generar_informe", description: "Crea un informe médico. El contenido DEBE estar en prosa narrativa: párrafos con oraciones completas (ej. 'Paciente femenino de 30 años, identificada bajo la cédula X, quien acude a consulta el... En la consulta se establece el diagnóstico de... Se indica el plan de...'). Para generarlo debes haber llamado antes a get_patient_data y obtener_historial; con esos datos redactas el contenido. NO incluyas el nombre del médico en el contenido. Si obtuviste un historial con historial_incompleto y mensaje_recordatorio: muestra solo ese recordatorio y espera. Si el usuario elige completar la historia, usa open_section. Si elige generar con su contenido, pide tipo y contenido y añade: «El informe se generará con el texto que me indiques; no incluirá datos de la consulta reciente porque no están completos en la historia.»", parameters: { type: "object", properties: { paciente_id: { type: "number" }, paciente_nombre: { type: "string", description: "Si no tienes paciente_id, indica el nombre del paciente" }, tipo_informe: { type: "string", description: "Ej: consulta, examen, general, control" }, contenido: { type: "string", description: "Texto del informe en prosa narrativa (HTML o texto), construido a partir de datos del paciente y del historial" }, observaciones: { type: "string" } }, required: ["tipo_informe", "contenido"] } } },
  { type: "function" as const, function: { name: "crear_recipe_medico", description: "Genera un PDF de récipe con medicamentos e indicaciones. Solo rol médico. Requisito: el paciente debe tener al menos una consulta completada, finalizada o con fecha de culminación registrada **contigo**; si la herramienta devuelve error por eso, explícalo al médico y ofrece open_section (historia-medica). Pide paciente (id o nombre), texto_recipe y texto_indicaciones antes de llamar. Tras éxito, el usuario descarga el PDF con el botón «Descargar PDF» en el chat.", parameters: { type: "object", properties: { paciente_id: { type: "number" }, paciente_nombre: { type: "string" }, texto_recipe: { type: "string", description: "Medicamentos, dosis, vía, duración" }, texto_indicaciones: { type: "string", description: "Orientación al paciente" }, fecha_emision: { type: "string", description: "YYYY-MM-DD opcional" } }, required: ["texto_recipe", "texto_indicaciones"] } } },
...
- Récipe PDF desde el chat: \`crear_recipe_medico\` solo con paciente + texto_recipe + texto_indicaciones. Si falla porque no hay consulta completada/finalizada contigo, explica el motivo y ofrece \`open_section\` (historia-medica). Tras éxito, menciona el botón **Descargar PDF**.
...
9) Récipe médico en PDF desde el chat: cuando el médico pida generar récipe, receta o indicaciones en PDF con paciente y textos, usa __ACTION__create_medical_recipe__{"paciente_nombre":"...","texto_recipe":"...","texto_indicaciones":"..."}__ (o paciente_id numérico). El sistema exige que el paciente tenga al menos una consulta completada o finalizada contigo; si falla, explica y ofrece open_section a historia médica.
...
- create_medical_recipe: {"paciente_nombre":"Nombre","texto_recipe":"medicamentos y dosis...","texto_indicaciones":"indicaciones al paciente..."} o con paciente_id. Requiere consulta completada/finalizada contigo; si no, el sistema devolverá error.
